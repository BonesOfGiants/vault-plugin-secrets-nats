...
bao write nats-secrets/issue/operator/myop/account/sys/user/default-push @sysaccount/default-push.json
bao write nats-secrets/issue/operator/myop/account/sys @sysaccount/sysaccount.json
bao write nats-secrets/issue/operator/myop @operator/operator.json

echo -e "${BGREEN}> Generate NATS server config with preloaded operator and sys account settings${NC}"    
OPERATOR_JWT=$(bao read -format=json nats-secrets/jwt/operator/myop  | jq -r .data.jwt)
SYSACCOUNT_PUBLICKEY=$(bao read -format=json nats-secrets/nkey/operator/myop/account/sys  | jq -r .data.publicKey)
SYSACCOUNT_JWT=$(bao read -format=json nats-secrets/jwt/operator/myop/account/sys  | jq -r .data.jwt)
...
bao write nats-secrets/issue/operator/myop/account/myaccount @account/myaccount.json
bao write nats-secrets/issue/operator/myop/account/myaccount/user/user @account/myuser.json
echo -e "${BGREEN}> Exporting user creds file${NC}"    
bao read -field creds nats-secrets/creds/operator/myop/account/myaccount/user/user > creds
